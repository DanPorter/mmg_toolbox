...
class ScanRangeSelector:
    ...
    def __init__(self, root: tk.Misc, initial_directory: str | None = None, config: dict | None = None,
        ...
        ttk.Label(frm, text='First:').pack(side='left', padx=2)
        var = ttk.Entry(frm, textvariable=self.number_start, width=8)
        var.pack(side='left', padx=2)
        var.bind("<Return>", self.update_numbers)
        ttk.Label(frm, text='Last:').pack(side='left', padx=2)
        var = ttk.Entry(frm, textvariable=self.number_end, width=8)
        var.pack(side='left', padx=2)
        var.bind("<Return>", self.update_numbers)
        ttk.Label(frm, text='Step:').pack(side='left', padx=2)
        var = ttk.Entry(frm, textvariable=self.number_step, width=4)
        var.pack(side='left', padx=2)
        var.bind("<Return>", self.update_numbers)
        ttk.Button(frm, text='Get numbers', command=self.numbers_from_exp).pack(side='left')
        ttk.Button(frm, text='Generate', command=self.update_numbers).pack(side='left', padx=4)
        ttk.Button(frm, text='Select Files', command=self.select_files).pack(side='left', padx=4)
        ttk.Button(frm, text='Find', command=self.find_scans).pack(side='left', padx=4)

        # Text box
        frm = ttk.Frame(self.root)
        frm.pack(side='top', expand=True, fill='both')

        ttk.Label(frm, text='Scans = ').pack(side='left', padx=2)
        self.text = tk.Text(frm, wrap='word', width=65, height=5)
        ...
        ttk.Button(frm, text='Check', command=self.show_metadata).pack(side='left', fill='y')
